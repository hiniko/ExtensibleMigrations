...
public sealed class MaterializedViewOperationHandler : IMigrationOperationHandler
{
    ...
    private static IEnumerable<(string Name, string Query)> Views(IRelationalModel? m)
    {
        ...
        {
            var n =
                et.FindAnnotation(MaterializedViewExtensions.ViewNameAnnotation)?.Value as string;
            var q =
                et.FindAnnotation(MaterializedViewExtensions.ViewQueryAnnotation)?.Value as string;
